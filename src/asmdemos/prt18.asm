    (* This is a Befunge kernel providing a subroutine 'P' to print
     * an unsigned 18-bit number. At the bottom of this source file
     * we initialize the stack to contain just the number we want
     * printed, and initialize Befunge-space to contain the program
     * "P@".
     *)
  ...
  .ORG (777,0)
   ...
   (************ Hardware Context Setup Code *************)
      ...
      NOP                      RET          SNZ.X $5       SNZ $5          
      NOP                      SW 3,[4]     DEC $1,$1      LW $5, [7]      { $3=stack index }
      NOP                      GOB          ADD.X 5,5,$PC  AND 7,7,5       { $4=stack base }
      LV $3, Stack             CALLV Push4  INC.s $3, $3   LMR $5, #HCAND  { $5=string char }
  ...
  (****** Handlers for '?' and '@' ******)
    ...
    (* This random number generator is a linear feedback shift
      ...
      v>vv><>v>^>^<<^<>>^v^>><vv<vv>^>v<^^^<><v><v^>>>v><>^><><><<<^<<
      ><v<<>^v<>>>v<<v^v>>^>v>^<^^<<>>v^<vvv^>^<>^^>vvv<>^>>>^>^><<<v<
      ^>^^<^>>^^vvv^<^^><v>>v<>v<vv<^vv^v^<<^>>v^>v>v<<^^>>vv>>>>><<^^
      <>>vv<>v>v><<^v<v>>^<vv^^v^^<^<><^>^v<<>vv^>v<v^<<v>vv<<vvvv><<v
      v^vv<^^v<v<><<>^^<v>v^<^v>^vv<v^><>>>^<^v<^>v^^^v<^<v<^<^<<<<><^
      <^><^<v>^^^^><^>vv^<v^v<><>><^^^>>^vv>v<>^<>v^vv>^<v^^<v<v<<<^><
      v<>><v^<>vvv>><>v<^v^^>^^>^v><vv>v>><<v^^>v^v^><<>v^^vv^^^^<<<>> *)
...
SMR.x $7, #OUTPUT     INC.s 7,7
SZ $0                 SNZ.y $3
WORD PrtBuf2+(0,1)    SHL.s 7,7
LW $3, [PC-DPC]       LW $7, [$6]
XOR 3,3,6             DEV.y 6,6         {4}
SNZ $3                SW $7, [$6]
LV.y $PC, PrtExit2    MR 7,3
LW.y $7, [$6]         SNZ.y $3
INV.y 6,6             SUB.s 3,7,3
LV.y $PC, ..y-11d     LI $3, #10d 
TRP 0                 INC.s 7,7
                      ...
                      SZ.y $3
                      AND 3,3,4
TRP 0                 SHL.s 7,7                                 .EQU PrtExit2 ..y
LV.y $PC, ..y+2       LW $7, [$6]       {0}
WORD 400000           LW $3, [-$PC]      
WORD PrtBuf2          LW $6, [-$PC]      
LW $7, [++$PC]        SHL.s 4,4            TRP 0
LW $6, [++$PC]        WORD 7               DEV $1,$1            .EQU PrtComp2 ..y
LI $3, ' '            WORD 6               SW $7, [--$PC]
SMR.x $3, #OUTPUT     WORD 0               SW $6, [--$PC]
RET                   WORD 0               SW $0, [--$PC]
                      WORD 0               SW $0, [--$PC]
